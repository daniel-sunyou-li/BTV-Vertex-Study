...
//     std::cout << "CP " << &cp <<  std::endl;
  ...
  for(unsigned int i=0;i<vertices->size();i++){
     ...
     double pv_x = pv.x(), pv_y = pv.y(), pv_z = pv.z();

     float s=sortingAlgo_.score(pv,pvToCandVector[i],useMET_);
     float s_jet=sortingAlgo_.score_jet(pv,pvToCandVector[i]);
     float s_met=sortingAlgo_.score_met(pv,pvToCandVector[i]);
     float s_lep=sortingAlgo_.score_lep(pv,pvToCandVector[i]);
