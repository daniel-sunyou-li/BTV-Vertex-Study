...
PrimaryVertexSorter<ParticlesCollection>::PrimaryVertexSorter(const edm::ParameterSet& iConfig) :
  ...
  tokenGenParticles_(consumes<reco::GenParticleCollection> (iConfig.getParameter<edm::InputTag>("genParticles"))),
...
//     std::cout << "CP " << &cp <<  std::endl;
  ...
  //data_file.open( "sort_score_pf.txt", std::ios_base::app );
  std::ofstream data_file;
  data_file.open( "sort_score_4D.txt", std::ios_base::app );
  char columns[150];
  int l = sprintf( columns, "%9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s, %9s\n", "RECO X", "RECO Y", "RECO Z", "RECO T", "GEN X", "GEN Y", "GEN Z", "TOT", "JET", "MET", "LEP" );
  std::cout << columns;
  data_file << columns;
  ...
  for(unsigned int i=0;i<vertices->size();i++){
     ...
     const Vertex & pv = (*vertices)[i];
     double p_x = p.vx(), p_y = p.vy(), p_z = p.vz();
     double pv_x = pv.x(), pv_y = pv.y(), pv_z = pv.z(), pv_t = pv.p4().t();
     float this_score[4];
     float s=sortingAlgo_.score((*vertices)[i],pvToCandVector[i],useMET_,this_score);

     vertexScoreOriginal[i]=s;
     scores.insert(std::pair<float,int>(-s,i));
     char buffer[150];
     int m = sprintf( buffer, "%9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f, %9.3f\n", pv_x, pv_y, pv_z, pv_t, p_x, p_y, p_z, this_score[0], this_score[1], this_score[2], this_score[3] );
     data_file << buffer;
     std::cout << buffer;
  }
